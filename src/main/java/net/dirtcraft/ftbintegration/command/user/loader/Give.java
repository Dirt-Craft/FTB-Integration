...
public class Give implements CommandExecutor {
    ...
    public CommandResult execute(@Nonnull CommandSource src, @Nonnull CommandContext args) throws CommandException {
        ...
        boolean success = database.addLoaders(target.getUniqueId(), value);
        ...
        if (src != target){
            src.sendMessage(SpongeHelper.formatText("%s %d loader-chunks to %s's balance.", result, value, target.getName()));
            Balance.showBalanceAsync(target, target.getCommandSource().orElse(null));
        }
        src.sendMessage(SpongeHelper.formatText("%s %d loader-chunks to your balance.", result, value, target.getName()));
