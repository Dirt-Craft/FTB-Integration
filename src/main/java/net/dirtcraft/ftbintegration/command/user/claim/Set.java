...
public class Set implements CommandExecutor {
    ...
    public CommandResult execute(@Nonnull CommandSource src, @Nonnull CommandContext args) throws CommandException {
        ...
        if (player instanceof ChunkPlayerInfo) {
            ...
            info.setExtraChunks(value, 0);
        }

        String result = success ? "&aSuccessfully set" : "&cFailed to set";
        if (src != target) {
            src.sendMessage(SpongeHelper.formatText("%s %s's claim-chunks balance to %d.", result, target.getName(), value));
            Balance.showBalanceAsync(target, target.getCommandSource().orElse(null));
        }
        src.sendMessage(SpongeHelper.formatText("%s your claim-chunks balance to %d.", result, target.getName(), value));
