...
public class Balance implements CommandExecutor {
    ...
    public CommandResult execute(@Nonnull CommandSource src, @Nonnull CommandContext args) throws CommandException {
        if (userManager == null) userManager = LuckPermsProvider.get().getUserManager();
    ...
    private static void showBalanceSync(User target, CommandSource recipient){
        ...
        if (playerData == null || info == null) {
            recipient.sendMessages(SpongeHelper.formatText("&cCould not retrieve ftb chunk balance.\n" +
                    "&cEither the user does not exist or are not in the database."));
            return;
        }
        LuckPermHandler.setPlayerBaseChunkData(info, userManager.getUser(target.getUniqueId()));
