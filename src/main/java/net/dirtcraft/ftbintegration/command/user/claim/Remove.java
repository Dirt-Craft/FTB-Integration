...
public class Remove implements CommandExecutor {
    ...
    public CommandResult execute(@Nonnull CommandSource src, @Nonnull CommandContext args) throws CommandException {
        ...
        if (src != target) {
            src.sendMessage(SpongeHelper.formatText("%s %d claim-chunks from %s's balance.", result, value, target.getName()));
            Balance.showBalanceAsync(target, target.getCommandSource().orElse(null));
        }
        src.sendMessage(SpongeHelper.formatText("%s %d claim-chunks from your balance.", result, value, target.getName()));
